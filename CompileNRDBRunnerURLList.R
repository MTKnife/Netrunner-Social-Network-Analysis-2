...
NRDB <- remoteDriver()
indexURLTemplate <-
  "http://netrunnerdb.com/en/decklists/find/PGIDX?sort=date&faction=Runner&lastpack=&author=&title="
URLElementList <- list()
deckURLList <- list()
newURLList <- list()
...
if (file.exists("NRDBRunnerURLList.txt")) {
  file.remove("NRDBRunnerURLList.txt")}
# Write the list of URL"s to a file.
invisible(lapply(deckURLList, write, "NRDBRunnerURLList.txt", append=TRUE))
