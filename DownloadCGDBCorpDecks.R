...
if (file.exists("CorpDecksDB.json")) {
  file.remove("CorpDecksDB.json")}
# Write the first line of the output database.
write("{ \"decks\": [", file = "CorpDecksDB.json")
# Load the list of deck URL's.
deckURLList <- readLines(con = "CorpURLList.txt")
...
for (deckIndex in 1:length(deckURLList)) {
  ...
  if (notFirstDeck) {write(",", file = "CorpDecksDB.json", append = TRUE)}
  ...
  deck <- sub("\\{",sub("\\}$",",",toJSON(list(deckinfo = list(name =
	...
	deckObject)
  # Write the new JSON object to the next line of the Corp decks database.
  write(deck, file = "CorpDecksDB.json", append = TRUE)
  notFirstDeck = TRUE
...
write("]}", file = "CorpDecksDB.json", append = TRUE)